    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {topMemories.map((memory, idx) => (
          <div
            key={memory.hash}
            className="group relative overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.03] p-4 backdrop-blur-sm transition-all duration-200 hover:border-white/[0.12] hover:bg-white/[0.05]"
          >
            {/* Relevance score bar */}
            <div className="absolute left-0 top-0 h-full w-1 rounded-full bg-gradient-to-b from-[#9945FF] to-[#00D1FF]" style={{ opacity: 0.4 + (memory.score || 0) * 0.006 }} />
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <div className="flex items-center gap-1 text-white/30">
                    ...
                    <span className="font-mono text-[11px]">{truncateHash(memory.hash)}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-0.5 text-white/30">
                    ...
                    <span className="text-[10px]">{formatDate(memory.timestamp)}</span>
                  </div>
                </div>
              ...
              {memory.verified && (
                <a
                  href={`https://explorer.solana.com/tx/${memory.hash}`}
                  target="_blank"
