...
export async function getMythosCryptoMarketReport(): Promise<MythosCryptoMarketReport> {
  ...
  const global = {
    marketCapUsd: globalRaw?.data?.total_market_cap?.usd ?? null,
    marketCapLabel: fmtUsd(globalRaw?.data?.total_market_cap?.usd ?? null),
    btcDominance: globalRaw?.data?.market_cap_percentage?.btc ?? null,
    btcDominanceLabel: globalRaw?.data?.market_cap_percentage?.btc ? `${globalRaw.data.market_cap_percentage.btc.toFixed(1)}%` : 'unavailable',
    volume24hUsd: globalRaw?.data?.total_volume?.usd ?? null,
    volume24hLabel: fmtUsd(globalRaw?.data?.total_volume?.usd ?? null),
    activeCryptos: globalRaw?.data?.active_cryptocurrencies ?? null,
  };
